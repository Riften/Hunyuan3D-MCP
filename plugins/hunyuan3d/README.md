...
Provides 15 service tools (19 tools total), a capability guide, configuration checks and shared job polling over STDIO.
...
All service tools use Tencent Cloud TC3 automatically; no submission `backend` argument is needed.

## Service tools

Every service tool exposes named business arguments at the top level of its MCP input schema,
including explanations, enums, defaults and constraints. Structured values such as `image`,
`file`, and `multi_view_images` retain their typed object/array schemas. No separate documentation is required to discover
...
| `hy3d_generate_model_from_image` | One reference image → textured model | `pro`, TC3 |
| `hy3d_generate_model_from_multiview` | Main/front image + 1–7 additional views → textured model; defaults to Pro 3.1 | `pro`, TC3 |
...
Remove the outer `request` wrapper and the submission `backend` argument. For example,
`{"request":{"prompt":"chair"},"backend":"tc3"}` becomes `{"prompt":"chair"}`.
The old wrapper and unknown fields are rejected. Query/wait arguments returned by submissions
remain compatible, including their `service` and `backend` routing fields.
Move calls with additional reference views from `hy3d_generate_model_from_image` to
`hy3d_generate_model_from_multiview`.

### Example: generate from multiple views

Call `hy3d_generate_model_from_multiview` with views of the **same object**:

```json
{
  "image": {"url": "https://example.com/front.png"},
  "multi_view_images": [
    {"view": "left", "image": {"url": "https://example.com/left.png"}},
...
The main `image` is required, as are 1–7 additional views. Each additional angle must be unique.
Pro 3.0 accepts `left`, `right`, `back`; Pro 3.1 (the default for this tool) also accepts
`top`, `bottom`, `left_front`, `right_front`. Each image accepts URL, local path, or base64.
This submits one `SubmitHunyuanTo3DProJob` with `MultiViewImages`, not one job per image.
Copy the returned query/wait arguments just as for single-image generation.
...
{
  "file": {
    "url": "https://example.com/generated-chair.glb",
    "type": "GLB"
  },
  "prompt": "棕色皮革坐垫和拉丝金属椅脚",
  "enable_pbr": true
}
```
...
{
  "file": {
    "url": "https://example.com/converted-model.fbx",
    "type": "FBX"
  }
}
```
